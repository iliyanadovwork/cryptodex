 ...
 *
 * This service has no public channels at all: its only socketEmitOne() call
 * site is notification.controller.js, which emits unread notifications into the
 * room named by a user _id. (socketEmitAll() is used for the two
 * verification pings, which go to everyone by design and carry no private
 * data beyond the userId being verified.) The helper is kept here so all four
 * socket services share one definition of "that name is somebody's private
 * room"; it is exported for the tests and for symmetry rather than because
 * this service has a `subscribe` handler - it does not.
 */
