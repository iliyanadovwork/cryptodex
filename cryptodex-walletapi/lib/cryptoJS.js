...
export function encryptValue(value) {
    const key = CryptoJS.enc.Utf8.parse(config.COIN_GATE_WAY.BTC.ENCRYPT_KEY);
    try {
        var encrypted = CryptoJS.AES.encrypt(value, key, {
            iv: CryptoJS.enc.Utf8.parse(config.COIN_GATE_WAY.BTC.IVVAR),
            mode: CryptoJS.mode.CBC,
        });
        return encrypted.toString();
    } catch (err) {
        console.log('err encryptObject', err);
        return null;
    }
...
export function decryptValue(value) {
    ...
    try {
        ...
        console.log('err decryptObject', err);
        return null;
    }
}